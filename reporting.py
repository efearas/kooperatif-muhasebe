...
def random_kisi_getir():
	...
	with connection.cursor() as cursor:
		...
		rows = []
		for row in cursor.fetchall():
			if row[0] not in [2,3,12,15,13,21,20,10]:
				rows.append([row[0], ])
...
def rapor_faturalar_kisi_fatura_detayi_yeni(_yil, _ay, _kisiID, page):
	...
	with connection.cursor() as cursor:
		...
		aTuple=(rows,float("{0:.2f}".format(toplam_kdvsiz)), toplam_kdv_1 ,toplam_kdv_8,toplam_kdv_18, float("{0:.2f}".format(toplam_genel)))
	return aTuple